...
#pragma omp parallel
  ...
  rlxd_init(2,  ((int) abs(t1*100.0))%20000000000);
  /* rlxd_init(2, 123456); */ 
  ...
  else
  {
    ...
    int accepted_cur = update();
      accepted += accepted_cur;
      printf("\t Step %04i,\t mp = %2.4lf,\t pl = %2.4lf,\t cc = %2.4lf,\t tc = %2.1lf,\t dh = %2.4lf,\tcg1 = %d,\tcg2 = %d,\t acc = %i,\t tacc = %lf\n", i, mean_plaquette(), polyakov_loop(), chiral_condensate(), topological_charge(), ham-ham_old, g_cgiterations1, g_cgiterations2, accepted_cur, ((double)accepted)/((double)i+1.0) );
      g_cgiterations1 = 0;
      g_cgiterations2 = 0;
